...
export const metadata: Metadata = {
  ...
  description: "Integrate scheduled webhooks into your app: API keys, the jobs API, the delivery contract, retries, and copy-pasteable examples.",
};
...
Dispatch is a hosted service — there is nothing to deploy. The API lives at:
  https://api-dispatch.imtiyazsayyid.in
...
Schedule a job:
  POST https://api-dispatch.imtiyazsayyid.in/api/v1/jobs
...
Cancel a job:
  DELETE https://api-dispatch.imtiyazsayyid.in/api/v1/jobs/:id  (same x-api-key header)
...
function Introduction() {
  return (
    <DocSection
      ...
      lead={
        <>
          Dispatch is a webhook scheduler: you give it a URL, a UTC timestamp, and an optional JSON payload, and it sends a <K>POST</K> to that URL at that moment. It replaces the cron box, the delay queue, and the retry
          loop you would otherwise build to make one HTTP request in the future.
        </>
      }
    />
  );
...
const QUICKSTART_STEPS: { title: string; body: React.ReactNode }[] = [
  {
    ...
    body: (
      <>
        <Link href="/register" className="text-amber-400 underline-offset-4 hover:underline">
          Register
        </Link>{" "}
        for Dispatch. Email, password, done — there is nothing to deploy or host.
      </>
    ),
  },
  {
    ...
    body: (
      <>
        In the dashboard, open <K>projects</K> and create one. Call it whatever the environment is — &quot;production&quot; is a fine first name.
      </>
    ),
  },
  {
    title: "Generate an API key",
    body: <>On the project page, generate a key and copy it when it is shown — that is the only time you will see it.</>,
  },
  {
    title: "Schedule your first job",
    body: (
      <>
        Point <K>webhookUrl</K> at any endpoint you control (or a request bin), set <K>fireAt</K> a couple of minutes out, and send it:
      </>
    ),
  },
  {
    ...
    body: (
      <>
        Open <K>jobs</K> in the dashboard. At <K>fireAt</K> the status lamp flips <K>SCHEDULED → FIRING → SUCCESS</K>, and the delivery log records the attempt — status code, response body, timing.
      </>
    ),
  },
];
...
function Quickstart() {
  return (
    <DocSection index={sectionIndex("quickstart")} id="quickstart" eyebrow="quickstart" title="Zero to a fired webhook" lead="Five steps, about five minutes, three of which you only ever do once.">
      <ol className="max-w-3xl">
        {QUICKSTART_STEPS.map((step, i) => (
          <li key={step.title} className="relative flex gap-5">
            <div className="flex flex-col items-center">
              <div className="grid size-9 shrink-0 place-items-center border border-zinc-800 bg-zinc-900/60 font-mono text-[11px] text-amber-400">{String(i + 1).padStart(2, "0")}</div>
              {i < QUICKSTART_STEPS.length - 1 ? <div className="w-px flex-1 bg-zinc-800" aria-hidden="true" /> : null}
            </div>
            <div className="min-w-0 flex-1 pb-8">
              <h3 className="pt-1.5 text-base font-medium text-zinc-100">{step.title}</h3>
              <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-zinc-400">{step.body}</p>
              {i === 3 ? (
                <div className="mt-4 space-y-3">
                  <CodeBlock code={QUICKSTART_CURL} lang="bash" />
                  <CodeBlock code={QUICKSTART_RESPONSE} lang="json" title="201 · response (abridged)" />
                </div>
              ) : null}
            </div>
          </li>
  ...
  return (
    ...
    >
      <div className="max-w-3xl space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">getting a key</p>
        <P>
          Keys are generated in the dashboard: open your project, then <K>api keys → new key</K>. Label it for where it will live — &quot;production server&quot;, &quot;CI&quot; — and copy it immediately. It is shown
          exactly once.
        </P>
      </div>

      <CodeBlock code={API_KEY_HEADER} lang="bash" title="passing the key" className="max-w-3xl" />

      <Callout tone="warn" legend="key security" className="max-w-3xl">
        <ul className="space-y-1.5">
          <li>
            Keys look like <K>sk_</K> followed by 64 hex characters and are stored as a SHA-256 hash — Dispatch cannot show a key again after creation.
          </li>
          <li>The dashboard shows only a label, the last four characters, and when the key was last used.</li>
          <li>Revoke a key at any time from the project page; revocation is immediate.</li>
          <li>Treat keys like passwords: environment variables, never client code, never the repo.</li>
        </ul>
      </Callout>
    </DocSection>
  );
...
const CREATE_JOB_REQ = `curl -X POST https://api-dispatch.imtiyazsayyid.in/api/v1/jobs \\
  -H "Content-Type: application/json" \\
  -H "x-api-key: sk_4f8a2c9b51de836b91c0a7e2d4f6a8b0" \\
  -d '{
...
function SchedulingAJob() {
  return (
    ...
    >
      <Endpoint method="POST" path="/api/v1/jobs" auth="api-key" description="Schedules a job. Returns the created job object with a 201.">
        <FieldList title="request body">
          ...
          <Field name="webhookUrl" type="string · url" required>
            The endpoint Dispatch will <K>POST</K> to. Must be a valid URL and reachable from wherever Dispatch runs.
          </Field>
          <Field name="fireAt" type="string · ISO 8601 UTC" required>
            When to fire, e.g. <K>2026-06-13T09:30:00Z</K>. Must be UTC (trailing <K>Z</K>) and in the future.
          </Field>
          <Field name="payload" type="object">
            Arbitrary JSON delivered in the webhook body. Defaults to <K>{"{}"}</K>.
          </Field>
        </FieldList>
      ...
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">what makes a valid fireAt</p>
        <P>
          An ISO 8601 timestamp in UTC with a trailing <K>Z</K> — <K>2026-06-13T09:30:00Z</K>. Offsets like <K>+05:30</K> are rejected with a 400, and so is any timestamp that isn&apos;t in the future. Dispatch arms the
          timer for the exact millisecond, so send the moment you mean, not a rounded-down minute.
        </P>
      </div>

      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">what payload is for</p>
        <P>
          Whatever JSON you put in <K>payload</K> comes back verbatim in the delivery body when the job fires. Use it to carry the context your handler needs — a user ID, a plan name — so the handler doesn&apos;t have to
          look anything up to know what the moment means. If you omit it, the delivery carries <K>{"{}"}</K>.
        </P>
      </div>

      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">what happens after you post</p>
        <P>
          The job is written to the database and a timer is armed for <K>fireAt</K>. The 201 response returns the full job object — keep <K>data.id</K> if you might cancel later. From there the job sits in{" "}
          <K>SCHEDULED</K> until it fires; you can watch it in the dashboard the whole way.
        </P>
      </div>
    </DocSection>
  );
  ...
  return (
    ...
    >
      <Endpoint
        ...
        description={
          <>
            Cancels a scheduled job. Only jobs in <K>SCHEDULED</K> state can be cancelled — it is the only state where there is still anything to call off.
          </>
        }
      >
        <div className="grid items-start gap-3 xl:grid-cols-2">
      ...
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">cancelling a job that already fired</p>
        <P>
          If the job is in any other state — already <K>SUCCESS</K>, mid-retry in <K>FAILED</K>, <K>DEAD</K>, or even <K>FIRING</K> at that instant — the request returns a 400 and changes nothing:
        </P>
        <CodeBlock code={CANCEL_JOB_400} lang="json" title="400 · response" className="max-w-3xl" />
        <P>A 404 means the job ID doesn&apos;t exist or belongs to a different project than the key you used.</P>
      </div>
    </DocSection>
  );
}
...
function WebhookDelivery() {
  return (
    <DocSection index={sectionIndex("webhook-delivery")} id="webhook-delivery" eyebrow="webhook delivery" title="What arrives at your endpoint" lead="Every delivery is a POST with a JSON body. The shape never varies.">
      <CodeBlock code={DELIVERY_BODY} lang="json" title="delivery body" className="max-w-3xl" />
      ...
      <FieldList title="fields">
        ...
        <Field name="payload" type="object" required>
          Exactly the JSON you passed at scheduling time; <K>{"{}"}</K> if you passed none.
        </Field>
      ...
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">headers</p>
        <CodeBlock code={DELIVERY_HEADERS} lang="text" title="request headers" className="max-w-3xl" />
        <P>
          <K>Content-Type: application/json</K> is the only header Dispatch sets. There is no signature header yet — see{" "}
      ...
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">the response dispatch expects</p>
        <P>
          Return any <K>2xx</K> to mark the delivery <K>SUCCESS</K>. Any other status — or a connection error — marks it <K>FAILED</K> and schedules a retry. Whatever body your endpoint returns is captured in the
          delivery log, so a short diagnostic string is worth returning; it is what you will read in the dashboard when something breaks.
        </P>
      </div>

      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">retries</p>
        <P>
          The retry clock is fixed: 30 seconds after the first failure, 1 minute after the second, 5 minutes as the configured ceiling — and a job is marked <K>DEAD</K> on its third failed attempt. Every attempt is
          logged with its status code, response body, and timing.
        </P>
      </div>

      <Callout tone="warn" legend="timeout · know the gap" className="max-w-3xl">
        Dispatch does not currently set an explicit timeout on the delivery request — it waits as long as the Node.js runtime&apos;s default fetch limits allow (several minutes). A strict delivery timeout is planned.
        Don&apos;t lean on the gap: respond within a few seconds and do slow work after acknowledging.
      </Callout>
    </DocSection>
  );
}
...
function JobLifecycle() {
  return (
    <DocSection index={sectionIndex("job-lifecycle")} id="job-lifecycle" eyebrow="job lifecycle" title="The state machine" lead="Six states, one loop. The same status lamps you see in the dashboard.">
      <LifecycleDiagram />

      <Panel className="max-w-3xl divide-y divide-zinc-800/50">
        {STATUS_NOTES.map(({ status, note }) => (
          <div key={status} className="grid gap-x-4 gap-y-1.5 px-5 py-3.5 sm:grid-cols-[8.5rem_minmax(0,1fr)] sm:items-baseline">
...
DISPATCH_URL = os.environ.get("DISPATCH_URL", "https://api-dispatch.imtiyazsayyid.in")
...
const dispatchURL = "https://api-dispatch.imtiyazsayyid.in"
...
const DISPATCH_URL = 'https://api-dispatch.imtiyazsayyid.in';
...
curl -X DELETE https://api-dispatch.imtiyazsayyid.in/api/v1/jobs/<job-id> \\
  -H "x-api-key: $DISPATCH_API_KEY"`;
...
function CodeExamples() {
  return (
    ...
    >
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">scheduling a job</p>
        <CodeTabs
          tabs={[
      ...
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">handling the webhook</p>
        <P>
          The contract is simple: return any 2xx and the delivery is <K>SUCCESS</K>; return anything else — or be unreachable — and Dispatch retries with backoff. Two rules follow from it: respond before you do the heavy
          work, and make your handler idempotent, because retries mean a delivery can arrive more than once.
        </P>
        <CodeTabs
          tabs={[
      ...
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">verifying the sender</p>
        ...
        <CodeBlock code={HMAC_PLANNED} lang="bash" title="planned · x-dispatch-signature" />
        <P>In the meantime, don&apos;t accept anonymous POSTs: put a secret in the webhook URL when you schedule, and check it on receipt. It&apos;s not a signature, but it stops drive-by requests cold.</P>
        <CodeBlock code={TOKEN_VERIFY} lang="js" title="interim · shared-token check" />
      </div>
    </DocSection>
  );
}
...
const BASE_URLS = `https://api-dispatch.imtiyazsayyid.in/api/v1`;
...
const LIST_JOBS_REQ = `curl "https://api-dispatch.imtiyazsayyid.in/api/v1/jobs?status=SCHEDULED&limit=20" \\
...
function ApiReference() {
  return (
    ...
    >
      <div className="space-y-3">
        <p className="font-mono text-[10px] tracking-[0.2em] text-zinc-600 uppercase">base url</p>
        <CodeBlock code={BASE_URLS} lang="bash" title="base url" className="max-w-3xl" />
        <P>
          Dispatch is a hosted service — that base URL is all you need. (Running your own instance instead? The API is identical; swap in your own origin and see the{" "}
          <Link href="/docs/self-hosting" className="text-amber-400 underline-offset-4 hover:underline">
            self-hosting guide
          </Link>
          .)
        </P>
        <P>
          Every response — success or error — uses the same envelope. On errors, <K>status</K> is <K>false</K> and <K>message</K> says why, alongside the HTTP status code (400 for validation, 401 for bad credentials, 404
          for resources that aren&apos;t yours).
        </P>
        <CodeBlock code={ENVELOPE} lang="json" title="response envelope" className="max-w-3xl" />
      </div>
      ...
      <Endpoint
        ...
        description={
          <>
            Schedules a job and returns the created job object with a 201. Documented in full in{" "}
            <a href="#scheduling-a-job" className="text-amber-400 hover:underline">
              Scheduling a job
            </a>
            .
          </>
      ...
      <Endpoint
        ...
        description={
          <>
            Cancels a <K>SCHEDULED</K> job; 400 for any other state. Documented in full in{" "}
            <a href="#cancelling-a-job" className="text-amber-400 hover:underline">
              Cancelling a job
            </a>
            .
          </>
      ...
      <Endpoint method="GET" path="/api/v1/jobs" auth="bearer" description="Lists jobs across all of your projects, newest first. This is what the dashboard's jobs page calls.">
        <FieldList title="query parameters">
          <Field name="status" type="enum">
            Filter by status: <K>SCHEDULED</K>, <K>FIRING</K>, <K>SUCCESS</K>, <K>FAILED</K>, <K>DEAD</K>, or <K>CANCELLED</K>.
          </Field>
      ...
      <Endpoint
        ...
        description={
          <>
            Fetches one job with its full delivery history. <K>logs</K> is ordered newest first; each entry records the attempt number, outcome, response code, and whatever body your endpoint returned.
          </>
        }
      >
        <CodeBlock code={GET_JOB_RES} lang="json" title="200 · response (abridged)" className="max-w-3xl" />
      </Endpoint>
    </DocSection>
  );
}
...
export default function UsingDispatchPage() {
  return (
    <DocsShell crumb="using" title="Using Dispatch" lead="Integrate scheduled webhooks into your application — from first API key to production delivery." sections={SECTIONS}>
      <AgentPrompt
